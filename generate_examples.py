...
class utils:
    ...
    @staticmethod
    def split_on_users(X, y, n_valid_users, pick_random=False):
            user_indices = {}
            for i in range(y.shape[0]):
                user_id = utils.one_hot_to_index(y[i])
                if user_id not in user_indices:
                    user_indices[user_id] = []
                user_indices[user_id].append(i)
            
            all_users = list(user_indices.keys())
            if pick_random:
                valid_users = random.sample(all_users, n_valid_users)
            else:
                valid_users = all_users[:n_valid_users]
            ...
            X_v = X[valid_indices]
            y_v = y[valid_indices]
            X_u = X[unknown_indices]
            y_u = y[unknown_indices]
            
            return X_v, y_v, X_u, y_u
    ...
    def split_per_user(X, y, train_frac, valid_frac, test_frac, shuffle=False):
            ...
            for user_id, indices in user_indices.items():
                ...
                X_valid.extend(X[valid_indices])
                y_valid.extend(y[valid_indices])
...
# Constants
FEATURE_LENGTH = 5  # DU, DD, DU, UD, UU timing features
...
def create_examples(input_path, data_file, example_length):
    ...
    """
    # Get all user files, ignoring hidden files
    all_files = os.listdir(input_path)
    user_files = [f for f in all_files if not f.startswith('.') and f.endswith('.txt')]
    
    print(f"Found {len(all_files)} files in directory")
    print(f"Filtered to {len(user_files)} .txt files")
    
    n_users = len(user_files)
    print(f"Processing {n_users} user files")
    ...
    for i, user_file_name in tqdm(enumerate(user_files)):
        file_path = os.path.join(input_path, user_file_name)
        examples_from_file = 0
        
        try:
            with open(file_path, "r") as user_file:
                example = []
                for line_num, line in enumerate(user_file, 1):
                    # Skip empty lines
                    if not line.strip():
                        continue
                        
                    # Extract the 5 timing values
                    ...
                    for part in parts:
                        if ':' in part:
                            ...
                            try:
                                ...
                                print(f"Warning: Could not convert '{value_str}' to float in {user_file_name}:{line_num}")
                                features.append(0.0)
                        else:
                            try:
                                value = float(part)
                                ...
                                print(f"Warning: Could not parse '{part}' as float in {user_file_name}:{line_num}")
                                features.append(0.0)
                    
                    # Check if we have exactly 5 features
                    if len(features) == FEATURE_LENGTH:
                        if any(np.isnan(features)) or any(np.isinf(features)):
                            continue
                        example.append(features)
                    else:
                        print(f"Warning: Expected {FEATURE_LENGTH} features, got {len(features)} in {user_file_name}:{line_num}")
                        continue
                ...
                # Check if file had enough data
                if examples_from_file == 0:
                    skipped_files.append((user_file_name, "Insufficient data"))
                    print(f"Warning: File {user_file_name} had no complete examples")
                    
        except Exception as e:
            skipped_files.append((user_file_name, f"Error: {str(e)}"))
            print(f"Error processing {user_file_name}: {str(e)}")
            continue

    print(f"\nSuccessfully processed {len(user_files) - len(skipped_files)} files")
...
def parse_args(args):
    ...
    n_users = len([f for f in os.listdir(args.input_path) if not f.startswith('.') and f.endswith('.txt')])
    ...
    if os.path.isfile(os.path.join(args.output_path, data_file_name)):
...
def main():
    ...
    data_file = h5py.File(os.path.join(args.output_path, data_file_name), "w")
    ...
    print("Datasets are saved in: {}".format(os.path.join(args.output_path, data_file_name)))
